...
; checks
(check-within (update-acceleration (make-vector 10 0) (make-vector 0 0)) (c*vec GRAVITY (make-vector -1/100 0)) 1e-10)
(check-within (update-acceleration (make-vector 0 5) (make-vector 0 -5)) (c*vec GRAVITY (make-vector 0 -1/100)) 1e-10)
(check-within (update-acceleration (make-vector -3 12) (make-vector 2 0)) (c*vec GRAVITY (make-vector 5/2197 -12/2197)) 1e-10)
...
; checks
(check-within (inverse-sqr (make-vector 10 0)) (make-vector 1/100 0) 1e-10) ; checks
...
; a function that updates the rocket information
  (make-satellite (+vec (satellite-pos rkt) (satellite-vel rkt))
                  ...
                  (cond [(< (normalize (-vec (satellite-pos rkt) ORIGIN)) EARTHRADIUS) BOOM!]
...
; render an image of the rocket flying around, and the orbiting moon
  (place-image (satellite-image (constellation-craft sats))
               ...
               (place-image (satellite-image (constellation-moon sats))
                            ...
                            (place-image EARTH (vector-x ORIGIN) (vector-y ORIGIN)
                                         THEVOID))))
;checks
